...
extension HttpClient {
    ...
    func upload(aUrlString: String,
       ...
       let request = Alamofire.upload(URL.init(fileURLWithPath: resourcePath), to: aUrlString, method: .post, headers: header).uploadProgress { (progress) in
        ...
        .responseJSON { (responseData) in
            ...
            if result.isSuccess {
                print("*****上传成功：\(String(describing: result.value))")
                completion(result.value ?? "", result.error)
            } else {
                print("*****上传失败：\(String(describing: result.error?.localizedDescription))")
    ...
    private func request(aMethod: HTTPMethod = .get,
        ...
        request.responseData(completionHandler: { (responseData) in
            ...
            HttpClient.shareInstance.requestTable.removeValue(forKey: (request.task?.taskIdentifier)!)
