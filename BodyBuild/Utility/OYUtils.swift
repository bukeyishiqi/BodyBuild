...
public class OYUtils {
    ...
    // MARK: 截取链接中的字段值
    class func resolveLink(link: String, SeparatedString: String) -> String? {
        let arr = link.components(separatedBy: SeparatedString)
        
        if arr.count > 1 {
            let subString = arr[1]
            let subArr = (subString as NSString).components(separatedBy: "&")
            return subArr[0]
        }
